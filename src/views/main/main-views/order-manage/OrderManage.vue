...
<template>
  <div class="search">
          ...
          <Row @keydown.enter.native="handleSearch">
            <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form">
              ...
              <span v-if="drop">
                ...
                <Form-item label="商品数量" prop="number">
                  <Input
                    type="text"
                ...
                <Form-item label="订单总价" prop="money">
                  <Input
                    type="text"
          ...
          <Row class="operation">
            ...
            <Dropdown @on-click="handleDropdown">
              ...
              <DropdownMenu slot="list">
                ...
                <DropdownItem name="reset">重置订单密码</DropdownItem>
    ...
    <Drawer title="导入数据" closable v-model="importModalVisible" width="1000">
      ...
      <Alert type="warning" show-icon>导入前请下载查看导入模版数据文件，确保数据格式正确，不得修改列英文名称。必需数据字段：订单名(唯一)、密码(明文)</Alert>
      <Table
    ...
    <!-- 验证密码 模态框:用于进行一些敏感操作前的身份验证如重置某个订单的密码之前需要验证管理员身份 -->
...
// 模版导入文件表数据
import { orderColumns, orderData } from "@/libs/importTemplate";
// 指定导出列数据
...
export default {
  ...
  methods: {
    ...
    handleDropdown(name) {
      if (name == "refresh") {
        ...
        if (this.selectCount <= 0) {
          this.$Message.warning("您还未选择要重置密码的订单");
          return;
        }
    ...
    resetPass() {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          resetOrderPass({ ids: ids }).then(res => {
            this.$Modal.remove();
            if (res.success) {
    ...
    importData() {
      ...
      importOrderData(this.importTableData).then(res => {
    ...
    enable(v) {
      this.$Modal.confirm({
        title: "确认启用",
        content: "您确认要启用订单 " + v.ordername + " ?",
        loading: true,
        onOk: () => {
          enableOrder(v.id).then(res => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.getOrderList();
    ...
    disable(v) {
      this.$Modal.confirm({
        title: "确认禁用",
        content: "您确认要禁用订单 " + v.ordername + " ?",
        loading: true,
        onOk: () => {
          disableOrder(v.id).then(res => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.getOrderList();
    ...
    remove(v) {
      this.$Modal.confirm({
        title: "确认删除",
        content: "您确认要删除订单 : " + v.content + " ?",
        loading: true,
        onOk: () => {
          deleteOrder(v.id).then(res => {
            this.$Modal.remove();
            if (res.success) {
    ...
    dealWith(v) {
      this.$Modal.confirm({
        title: "确认处理",
        content: "您确认要处理订单 : " + v.content + " ?",
        loading: true,
        onOk: () => {
          dealWithOrder(v.id, { id: v.id }).then(res => {
